...
# 盘中和盘后轮流下载所有股票的实时数据，目前只下载 逐笔 数据，报价、摆盘数据只能当时获取，不能获取当天的历史数据，
# 其他数据如 K 线、报价、分时 数据量不大，可以盘后下载，盘中即使下载了，实时分析时还需要更新一次，不划算
...
subscribe_subtypes = [ ft.SubType.TICKER ]
...
def get_ticker_data(market, scope):
	if market == "HK":
		if scope == "all":
